package com.gapatmej.predictor.web.rest.util;
...
public interface ResponseUtil {
     ...
     *
     * @param <X>           type of the response
     * @param maybeResponse List response to return if present
     * @return response containing {@code maybeResponse} if present or {@link HttpStatus#NOT_FOUND}
     */
    public static <X> ResponseEntity<List<X>> wrapOrNotFoundList(Optional<List<X>> maybeResponse) {
        return wrapOrNotFoundList(maybeResponse, null);
    }
     ...
     * @return response containing {@code maybeResponse} if present or {@link HttpStatus#NOT_FOUND}
     */
    public static <X> ResponseEntity<List<X>> wrapOrNotFoundList(Optional<List<X>> maybeResponse, HttpHeaders header) {
        return maybeResponse.map(response -> ResponseEntity.ok().headers(header).body(response))
                .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }
